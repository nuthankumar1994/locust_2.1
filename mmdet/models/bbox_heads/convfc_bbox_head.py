...
class ConvFCBBoxHead(BBoxHead):
    ...
    def __init__(self,
        ...
        if self.with_cls:
            ...
            print("convf_bbox_head.py/ConvFCBBoxHead()", self.num_classes,self.num_counts)
        if self.with_reg:
    ...
    def forward(self, x):
        #Fixme: convfc_bbox_head/forward() torch.Size([3072, 256, 7, 7]) 0 2 False
        #print("convfc_bbox_head/forward()", x.shape, self.num_shared_convs, self.num_shared_fcs, self.with_avg_pool)
        ...
        if self.num_shared_fcs > 0:
            if self.with_avg_pool:
                x = self.avg_pool(x)
            x = x.view(x.size(0), -1)   #[3072, 256, 7, 7] == 12544 = 256*7*7 ==> [3072, 12544]
            # Fixme: convfc_bbox_head/forward()/num_shared_fcs torch.Size([3072, 12544])
            #print("convfc_bbox_head/forward()/num_shared_fcs", x.shape)
            for fc in self.shared_fcs:
                x = self.relu(fc(x))
                #print("convfc_bbox_head/forward()/fc", x.shape)
        # separate branches
        x_cls = x
        x_reg = x
        x_count = x               #Fixme: +
        # Fixme: convfc_bbox_head/forward()/x_cls torch.Size([3072, 1024])
        #print("convfc_bbox_head/forward()/x_cls", x_cls.shape)
