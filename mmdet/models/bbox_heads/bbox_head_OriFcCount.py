...
class BBoxHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_count=dict( 
                     type='SmoothL1Loss', beta=1.0, loss_weight=1.0, pick=-1)):   
        ...
        if self.with_count:  
            self.fc_count = nn.Linear(in_channels, 1)  #Fixme: + 
    ...
    def loss(self,
        ...
        losses = dict()
        #print('count_score==>',len(count_score),count_score.shape)
        #print('counts==>',len(counts),counts.shape)
        ...
        # print('bbox_head/count_score', counts.shape, count_score.shape, counts_np.shape)  #
        if count_score is not None:
            ...
            count_score = count_score.unsqueeze(1).type(torch.float)
            losses['loss_count'] = 0.0001*self.loss_count(    # Fixme: + Resized binary count  0.0001 ==> 0.01
               count_score,
               counts,
               count_weights,
               avg_factor=avg_factor,
    ...
    def get_det_bboxes(self,
        ...
        print('bbox_heads/bbox_head/get_det_bboxes',count_score.shape,count_score_list.shape)
        # print((scores.shape), type(scores))
        # print((cls_score.shape), type(cls_score) )
        # print((count_score.shape), type(count_score) )
        # print("=============")
