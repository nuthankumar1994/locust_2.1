...
class BBoxHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 with_image_cls=True,              #Fixme: Image level distribution
                 with_image_count = True,          #Fixme: Image level distribution
                 roi_feat_size=7,
                 in_channels=256,
                 num_classes=141,  #81,
                 num_counts=57,  #81,                 
                 ...
                 loss_image_cls=dict(
                     type='WeightedMultilabelLoss',
                     loss_weight=1.0),
                 loss_image_count=dict( 
                     type='WeightedMultilabelLoss',
                     loss_weight=1.0),
                 loss_cls=dict(
        ...
        self.with_image_cls = with_image_cls                  #Fixme: Image level distribution
        self.with_image_count = with_image_count              #Fixme: Image level distribution
        ...
        self.loss_image_count = build_loss(loss_image_count)         #Fixme: Image level distribution
        self.loss_image_cls = build_loss(loss_image_cls)             #Fixme: Image level distribution
        ...
        if self.with_count:  
            self.fc_count = nn.Linear(in_channels, num_counts)  #Fixme: +  Resized binary count 1==>7==>57(56+1)
        if self.with_image_cls:
            self.fc_image_cls = nn.Linear(in_channels, num_classes)   #Fixme: + Image level distribution
        if self.with_image_count:  
            self.fc_image_count = nn.Linear(in_channels, num_counts)  #Fixme: +  Resized binary count 1==>7==>57(56+1)
    ...
    def init_weights(self):
        ...
        if self.with_image_cls:                                    #Fixme: + Image level distribution
            nn.init.normal_(self.fc_image_cls.weight, 0, 0.01)
            nn.init.constant_(self.fc_image_cls.bias, 0)
        if self.with_image_count:                                  #Fixme: + Image level distribution
            nn.init.normal_(self.fc_image_count.weight, 0, 0.001)
            nn.init.constant_(self.fc_image_count.bias, 0)
    ...
    def AnchorGt2ImageGtForCounts(self, Counts):
        ''' AnchorGt2ImageGt. Counts.shape = torch.Size([3072]) 
            Image_Counts.shape = torch.Size([6,57])
        '''
        Counts_np = Counts.cpu().numpy()
        GtCountsDiv_list = Counts_np.copy()
        for idx, count in enumerate(Counts_np):
        ...
        cnt_list = []
        Image_Counts = []
        for idx, count in enumerate(GtCountsDiv_list):
            if Flag < 512:
                Flag+=1
                cnt_list.append(count)
            else:
                Flag = 0
                Image_Counts.append(set(cnt_list))
                cnt_list = []
        print("AnchorGt2ImageGtForCounts",Image_Counts)
        ImageGt_Counts = []
        for cnt_set in Image_Counts:
            cnt_list = [0 for e in range(57)]
            for i in cnt_set:
                cnt_list[i] = 1
            ImageGt_Counts.append(copy.deepcopy(cnt_list))
        print("AnchorGt2ImageGtForCounts",len(ImageGt_Counts))
        return GtCountsDiv_list  #GtCountBins_list
    ...
    def loss(self, Stage_i,    # Fixme: + Resized Coarse-to-fine division and loss. Add the stage Flag.
             ...
             bbox_pred,
             count_score,
             cls_img_score,         # Fixme: image level distribution 
             count_img_score,         # Fixme: image level distribution
             counts,
             count_weights,
             labels,
             label_weights,
        ...
        losses = dict()
        # bbox_head/loss()==> torch.Size([3072]) torch.Size([3072, 57]) torch.Size([3072]) torch.Size([3072, 4]) torch.Size([6, 141]) torch.Size([6, 57])
        print("bbox_head/loss()==>",counts.shape,count_score.shape,labels.shape,bbox_targets.shape, cls_img_score.shape, count_img_score.shape)  


        image_counts = self.AnchorGt2ImageGtForCounts(Counts)
...
#            Loss_count_wight=0.1
        ...
        if count_score is not None:
            ...
            losses['loss_count'] = Loss_count_wight*self.loss_count(
                count_score,
                counts,
                count_weights,
                avg_factor=avg_factor,
    ...
    def refine_bboxes(self, rois, labels, bbox_preds, pos_is_gts, img_metas):
        ...
        for i in range(len(img_metas)):
            ...
            keep_inds[:len(pos_is_gts_)] = pos_keepimport torch
import torch.nn as nn
import torch.nn.functional as F
from torch.nn.modules.utils import _pair
...
class BBoxHead(nn.Module):
    ...
    def AnchorGt2ImageGtForCounts(self, Counts):
        ...
        cnt_list = []
        Image_Counts = []
        num = len(GtCountsDiv_list)/512
        #print("AnchorGt2ImageGtForCounts",num)
        
        for i in range(int(num)):
            cnt_list = GtCountsDiv_list[i*512:(i+1)*512]
            Image_Counts.append(set(cnt_list))
        
        #print("AnchorGt2ImageGtForCounts",Image_Counts)
        ImageGt_Counts = []
        for cnt_set in Image_Counts:
            cnt_list = [0 for e in range(self.num_counts)]
            for i in cnt_set:
                cnt_list[i] = 1
            ImageGt_Counts.append(copy.deepcopy(cnt_list))
        #print("AnchorGt2ImageGtForCounts",len(ImageGt_Counts))
        return ImageGt_Counts  #GtCountBins_list


    def AnchorGt2ImageGtForLabels(self, Labels):
        ''' AnchorGt2ImageGtForLabels. Labels.shape = torch.Size([3072]) 
            Image_Labels.shape = torch.Size([6,141])
        '''
        Labels_np = Labels.cpu().numpy()
        GtLabelsDiv_list = Labels_np.copy()

        Flag = 0
        cnt_list = []
        Image_Labels = []
        num = len(GtLabelsDiv_list)/512
        
        for i in range(int(num)):
            cnt_list = GtLabelsDiv_list[i*512:(i+1)*512]
            Image_Labels.append(set(cnt_list))
        
        #print("AnchorGt2ImageGtForLabels",Image_Labels)
        ImageGt_Labels = []
        for cnt_set in Image_Labels:
            cnt_list = [0 for e in range(self.num_classes)]
            for i in cnt_set:
                cnt_list[i] = 1
            ImageGt_Labels.append(copy.deepcopy(cnt_list))
        #print("AnchorGt2ImageGtForLabels",len(ImageGt_Labels))
        return ImageGt_Labels  #GtCountBins_list
    ...
    def loss(self, Stage_i,    # Fixme: + Resized Coarse-to-fine division and loss. Add the stage Flag.
        ...
        image_counts = torch.from_numpy(np.array(self.AnchorGt2ImageGtForCounts(counts),np.float32)).cuda()
        image_labels = torch.from_numpy(np.array(self.AnchorGt2ImageGtForLabels(labels),np.float32)).cuda()
        # bbox_head/loss()==> torch.Size([6, 57]) torch.Size([3072]) torch.Size([3072, 4]) torch.Size([6, 141])
        #print("bbox_head/loss()==>",image_counts.shape,label_weights.shape,bbox_weights.shape,image_labels.shape)

        if cls_img_score is not None:
            avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
            losses['loss_image_cls'] = self.loss_image_cls(
                cls_img_score,
                image_labels)
            #losses['acc_image_cls'] = accuracy(cls_img_score, image_labels)

        if count_img_score is not None:
            avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
            losses['loss_image_count'] = self.loss_image_count(
                count_img_score,
                image_counts)
            #losses['acc_image_count'] = accuracy(count_img_score, image_counts)
