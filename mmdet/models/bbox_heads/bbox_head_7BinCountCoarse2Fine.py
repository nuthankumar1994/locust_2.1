...
class BBoxHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_count=dict( 
                     type='SmoothL1Loss', beta=1.0, loss_weight=1.0, pick=-1)):    #Fixme:'MSELoss', loss_weight=1.0   ||   'SmoothL1Loss', beta=1.0, loss_weight=1.0, pick=-1
        ...
        if self.with_count:  
            self.fc_count = nn.Linear(in_channels, 8)  #Fixme: +  Resized binary count 1==>7
    ...
    def Stage1Div(self, Counts):
        ...
        for idx, count in enumerate(Counts_np):
            if count ==0:
                ...
                GtCountsDiv_list[idx] = 64
            else:
                GtCountsDiv_list[idx] = 128
    ...
    def MatchCountWeights(self, CountWeights):
        ...
        GtCountBins_list = []
        for Val in GtCountsDiv_list:
            #GtCountBins_list.append([Val,Val,Val,Val,Val,Val,Val])
            GtCountBins_list.append([14.0,12.0,10.0,8.0,6.0,4.0,2.0,1.0])
        return GtCountBins_list


    def int2bin(self, Val):
        bin_list = [0,0,0,0,0,0,0,1]
        Valb = bin(Val)
        Valb = Valb[2:].zfill(8)  #填充0，最长8位，
    ...
    def loss(self, Stage_i,    # Fixme: + Resized Coarse-to-fine division and loss. Add the stage Flag.
        ...
        if count_score is not None:
            ...
            losses['loss_count'] = 0.001*self.loss_count(     # Fixme: + Resized binary count  0.0001 ==> 0.01 ==>0.001
               count_score,
               counts,
               count_weights,
               avg_factor=avg_factor,
